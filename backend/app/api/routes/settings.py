...
def update_settings(update: SystemSettingsUpdate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    for field, value in update.model_dump(exclude_unset=True).items():
        if not value and field in ['whatsapp_access_token', 'facebook_access_token']:
            continue
...
async def backfill_conversation_links(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    convs = db.query(Conversation).filter(Conversation.cached_conversation_link == None).all()
    if not convs:
